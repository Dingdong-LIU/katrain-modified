...
class BaseGame:
    ...
    DEFAULT_PROPERTIES = {"GM": 1, "FF": 4}  # CirF: Game type and SGF standard specification
...
class Game(BaseGame):
    ...
    def __init__(
        self,
        katrain,
        engine: Union[Dict, KataGoEngine],  # CirF: Union[X, Y] means either X or Y
        move_tree: GameNode = None,
        analyze_fast=False,
        game_properties: Optional[Dict] = None,  # CirF: Optional[X] is equivalent to Union[X, None]
        sgf_filename=None,
    ):
        super().__init__(
        ...
        self.player_predict_url = "http://143.89.191.114:7130/predict"
        ...
        def analyze_and_play(node):
            ...
            if move.is_pass:
                if self.current_node == cn:
                    self.set_current_node(node)
                return
            new_node = GameNode(parent=node, move=move)  # CirF: New node creation
