...
def fmt_moves(moves: List[Tuple[float, Move]]):
    """
    Covert a list of moves to a string representation, composed of the move's GTP coordinate and the move's probability.

    Args:
        moves (List[Tuple[float, Move]]): List of moves to convert.

    Returns:
        str: String representation of the moves. For example, "A1 (0.5%), B2 (0.3%), C3 (0.2%)".
    """    
...
def generate_ai_move(game: Game, ai_mode: str, ai_settings: Dict) -> Tuple[Move, GameNode]:
    ...
    if (ai_mode in AI_STRATEGIES_POLICY) and cn.policy:  # pure policy based move
        ...
        ai_thoughts += f"Using policy based strategy, base top 5 moves are {fmt_moves(policy_moves[:5])}. "  # CirF: Remember this print
        ...
        if top_5_pass:  # CirF: 1 - Check for pass
            ...
            if ai_mode == AI_RANK:  # calibrated, override from 0.8 at start to ~0.4 at full board  # CirF: 3 - Compute thresholds
    ...
    # CirF: 6 - Return move and thoughts
    game.katrain.log(f"AI thoughts: {ai_thoughts}", OUTPUT_DEBUG)
    played_node = game.play(aimove)
    played_node.ai_thoughts = ai_thoughts
    return aimove, played_node

# CirF: generate_helper_advice()
def generate_helper_advice(game: Game, ai_mode: str, ai_settings: Dict):
    cn = game.current_node
    if not (ai_mode == AI_RANK and cn.policy):
        return
    ...
    if cn.generate_advice:
        ...
        if cn.analysis and cn.analysis["moves"] and len(cn.analysis["moves"]) > 0:
            ...
            cn.cognitive_depth_p = top_node_visits/sum(visits_list)
